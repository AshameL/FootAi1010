    ...
    query.find({
        success: function(results) {
            // 循环处理查询到的数据
           	...
           	//var countResult = results.reverse();
            for (var i = 0; i < 1; i++) {
				pointDatas[0][i]=[];
				pointDatas[1][i]=[];
				...
				console.log(fileNameArr[2].substring(1,13));
...
function search(){
	...
	//格式化时间
    var formatedStartTime = formatDateStr(startTime);
    var formatedEndTime = formatDateStr(endTime);

    //根据当前用户id获取全量footdata数据
    $(".nowpage").html("1");
    Bmob.initialize("6ff39cf89b2961d80c536401f109dfa7", "2e540f69b92c8f9337dd1ae7d8e14ab7");
    var objectId = document.cookie.split(";")[0].split("=")[1];
    alert(objectId);
    ...
    query.find({
        success: function(results) {
			...
			for(var y = 0; y < results.length; y++){
                var matchfileName = results[y].get('fileName').split('-');
                var testDate = matchfileName[2].substring(1,13);
                if(testDate >= formatedStartTime && testDate <= testDate){
                    matchedResult[arrIndex] = results[y];
                    arrIndex++;
                }
			}
            arrIndex = 0;
            for (var i = 0; i < 1; i++) {
                pointDatas[0][i]=[];
                pointDatas[1][i]=[];
                var fileNameArr = matchedResult[i].get('fileName').split('-');
                userName = fileNameArr[0];
                modeStrings[i] = fileNameArr[1];
                dateStrings[i] = fileNameArr[2].substring(1,13);
                console.log(fileNameArr[2].substring(1,13));
                var pointDataArr = matchedResult[i].get('fileDatas');
                var j=0;
                var index=-1;
                var DT = '';
                while(j<pointDataArr.length){
...
function formatDateStr(timeStr){
    ...
    var dayStr = timeArr[0].replace(/-/g, "");
    var hourStr = timeArr[1].replace(/:/g, "").substr(0,4);
    var formatedTimeStr = dayStr + hourStr;
